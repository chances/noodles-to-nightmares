...
- **Global Declarations**: Variables (`var` block) and memory allocation (`Alloc()`)
...
**Ref**: [GTA Limits - Sanny Builder](https://docs.sannybuilder.com/scm-documentation)
...
**Rule**: **PREFER `gosub` over `goto` when control-flow is intended to return back to the call-site.** Use `gosub` and `return` for cleaner, more maintainable code.
...
**Legacy Rule** (if using `goto` for subroutines): Always provide an explicit return label and ensure proper flow when using `goto` for subroutines.
...
**Ref**: [Loops - Sanny Builder](https://docs.sannybuilder.com/language/control-flow)

### Mission Launch Context
**Critical Rule**: `Mission.LoadAndLaunchInternal()` must be called from **within** the main loop's `while true` block.
...
**Why**: GTA3's mission system expects continuous main loop execution. Calling mission load outside loop context causes script state corruption and crashes.
...
**Ref**: [0417: LOAD_AND_LAUNCH_MISSION_INTERNAL - GTAMods Wiki](https://www.gtamodding.com/index.php?title=0417)
...
- **Termination**: `terminate_this_script` ends mission and returns control to main
...
- **Usage**: Allocate memory handles for object references (vehicles, peds, blips)
...
This script checks mission conditions every 250ms, allowing smooth gameplay without excessive polling.
...
**Critical Rule**: NEVER delete user `TODO` and `FIXME` comments unless you are completing or fixing the respective issue.
...
**Why**: TODO and FIXME comments represent known issues, future improvements, or design decisions. They:
- Communicate intent to other developers
- Track technical debt and work items
- Should only be removed when the issue is resolved or explicitly documented as addressed

### Collaboration Protocol
**Critical Rule**: ALWAYS read files for human changes before making your own edits.
...
**Why**: You are a tool assisting a human with real intelligence. The human makes architectural and design decisions. You must respect their decisions and understand context before making changes.
